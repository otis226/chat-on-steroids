...
function coreInstructions(ctx: ToolContext, platform: NodeJS.Platform): string {
  ...
  if (caps.command) {
    lines.push(
      ...
      'A running command returns a session_id. Continue that same process with write_stdin and inspect its terminal result before reporting completion. For non-interactive builds/tests/package jobs, prefer one write_stdin wait_for="exit" with a suitable yield_time_ms instead of polling every log chunk. yield_time_ms is only a safety ceiling: process exit returns immediately, so never add sleeps/backoff just to reduce requests. Use wait_for="output" only for interactive input or when an intermediate progress read is actually useful. After a transient wait failure, keep the same session instead of starting replacement work.',
